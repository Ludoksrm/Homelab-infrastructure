...
## 🛠️ Rappels pour l'installation & Maintenance

Une fois les fichiers créés sur le serveur, suivez ces étapes pour finaliser l'installation :

1. **Droit d'exécution** : Donnez la permission au système de lancer le script :
   ```bash
   chmod +x /root/scripts/backup_configs.sh---
   ````
2. **est manuel** : Lancez-le une fois à la main pour valider la configuration et la connexion SSH :
    ```bash
    /bin/bash /root/scripts/backup_configs.sh
    ```
3. **Vérification des Logs** : Consultez le résultat de la synchronisation à tout moment :
   ```bash
